...
export const ActivityLog = () => {
    ...
    return (
        <div className="font-mono-tech text-xs h-full bg-black/80 backdrop-blur-sm flex flex-col overflow-hidden relative">
            {/* Filter Bar */}
            <div className="p-2 border-b border-zinc-800 flex flex-nowrap gap-2 overflow-x-auto custom-scrollbar sticky top-0 bg-black/90 z-10 shrink-0">
                {['ALL', 'BURN', 'LP_ZAP', 'REVSHARE', 'HARVEST', 'ANALYSIS'].map((f) => (
                    <button
                        key={f}
                        onClick={() => setFilter(f)}
                        className={`px-4 py-2 md:px-3 md:py-1 rounded text-[11px] md:text-[10px] font-bold tracking-wider transition-all border whitespace-nowrap ${filter === f
